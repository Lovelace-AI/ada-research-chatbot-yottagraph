...
An MCP-wired ADK agent that uses Elemental MCP tools to investigate
companies, executives, government entities, relationships, and events
in the Lovelace Knowledge Graph.

All data access goes through the Elemental MCP server — no direct
REST calls to the Query Server.
...
def _load_mcp_url() -> str:
    ...
    return os.environ.get(
        "ELEMENTAL_MCP_URL",
        "https://broadchurch-portal-194773164895.us-central1.run.app"
        "/api/mcp/org_6qKPf3xyZOET9N5z/elemental/mcp",
    )
...
_tools: list = [
    McpToolset(connection_params=SseConnectionParams(url=ELEMENTAL_MCP_URL)),
]
